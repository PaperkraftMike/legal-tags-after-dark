...
export default function NightOwlFlashSale() {
  ...
  useEffect(() => {
    const calcExpiry = () => {
      ...
      // Active window: 18:00 (6 PM) MST to 06:00 (6 AM) MST next day
      const isActive = mstHour >= 18 || mstHour < 6;

      if (!isActive) {
        if (!isPreview) { setOfferExpired(true); return; }
        // Preview mode: fake 6 hours remaining
      ...
      // Calculate ms until 6 AM MST
      let targetMst = new Date(mstNow);
      if (mstHour >= 18) {
        // Evening: target is 6 AM tomorrow MST
        targetMst.setDate(targetMst.getDate() + 1);
        targetMst.setHours(6, 0, 0, 0);
      } else {
        // After midnight: target is 6 AM today MST
        targetMst.setHours(6, 0, 0, 0);
      }
  ...
  const handleSubmit = (e) => {
    ...
    // Night Owl $100-off coupon rides along only while the offer window is live.
    // SamCart coupons have no time schedule, so the countdown gates the discount here.
    const redirectUrl = offerExpired ? baseRedirectUrl : `${baseRedirectUrl}?coupon=NIGHTOWL100`;
    ...
    <div style={{
      ...
      <section style={{ padding: '32px 20px 0', textAlign: 'center' }}>
        <div style={{ maxWidth: '500px', margin: '0 auto' }}>
          ...
          <p className="mono" style={{ fontSize: '12px', color: '#c9a227', fontWeight: '500' }}>
            Only available 6 PM to 6 AM MST · Expires at sunrise
          </p>
      ...
      <section style={{ padding: '20px', background: '#0d0d1a' }}>
        <a href="https://www.google.com/search?q=Legal+Tags+Philipsburg+MT" target="_blank" rel="noopener noreferrer"
          onClick={() => clarityEvent('nightowl_flash_google_reviews_click')}
          style={{
      ...
      <footer style={{ background: '#080812', color: '#e8e4d9', padding: '40px 20px', textAlign: 'center' }}>
        ...
        <div className="mono" style={{ fontSize: '13px', marginBottom: '12px' }}>
          ...
          <a href="mailto:support@legaltags.com" onClick={() => clarityEvent('nightowl_flash_email_footer')} style={{ color: '#c9a227', textDecoration: 'none', fontWeight: '500' }}>✉ support@legaltags.com</a>
        </div>
